...
def get_activations_generate(model_s, model, batch_size=50, dims=2048,
    ...
    for i in range(0, num_images, batch_size):
        ...
        # noise_batch = torch.randn(size=(batch_size, model_s.z_dim)).to(device)
        images = model_s.sample(batch_size)
...
def generate_from_model(model, save_path, num_images, batch_size=32, device=torch.device("cpu")):
    ...
    while count < num_images:
        # noise_batch = torch.randn(size=(batch_size, model.z_dim)).to(device)
        generated = model.sample(batch_size)
...
def calc_fid_from_dataset_generate(dataset, model_s, batch_size, cuda, dims, device, num_images):
    with torch.no_grad():
        fid = calculate_fid_given_dataset(dataset, model_s, batch_size, cuda, dims, device, num_images)
    return fid
